...
class TetrisPiece(SparseGridLayout):
    ...
    def __init__(self, **kwargs):
        super(TetrisPiece, self).__init__(**kwargs)
...
class Tetris(SparseGridLayout):
    ...
    brick_wall = ListProperty([[]] * 42)  # self.rows + 2
    sound = SoundLoader.load('tetris.wav')
    piece_num = NumericProperty(0)
    piece_idx = NumericProperty(0)
    blocks = ListProperty([]*200)
    ...
    def __init__(self, **kwargs):
        ...
        # self.sound.play()
        self.piece_idx = 0
        parts = ['J', 'L', 'H', 'T', 'T', 'S', 'O', 'T', 'O', 'O', 'L', 'J', 'Z', 'S', 'Z']
        parts += ['T', 'I', 'J', 'L']  # T - right end
        parts += ['J', 'S', 'L', 'H', 'O', 'T', 'L']  # J - left end
        parts += ['S', 'L', 'S', 'T', 'T', 'L', 'O', 'O', 'S', 'Z']  # S - both ends, T - right, O - left
        parts += ['I', 'Z', 'O', 'Z', 'T', 'S', 'Z', 'S']  # S - right
        parts += ['L', 'H', 'L', 'I', 'S', 'Z', 'Z', 'S', 'H']  # S - right
        parts += ['Z', 'S', 'T', 'H', 'I', 'S', 'O']  # Z - left
        parts += ['J', 'Z', 'O', 'S', 'T', 'S', 'L', 'O']  # Z - left
        parts += ['L', 'L', 'O', 'J', 'T']
        parts += ['T', 'S', 'H', 'O', 'L']
        parts += ['J2', 'S', 'S', 'O']
    ...
    def set_falling(self):
        ...
        if self.falling_piece.name == 'J2':
            self.falling_piece.row = int(self.rows) - 2
        else:
            self.falling_piece.row = int(self.rows) - 1
        self.falling_piece.column = int(self.columns / 2)
    ...
    def set_next(self):
        ...
        if len(self.blocks) > self.piece_idx:
            random_name = self.blocks[self.piece_idx]
        else:
    ...
    def collide_shift(self):
        ...
        for brick in self.falling_piece.bricks:
            ...
            shift_pos = [anchor_pos[0] + pos['x'],
                         anchor_pos[1] + pos['y']]
            if shift_pos[1] < 0 or shift_pos[1] >= int(self.columns) or shift_pos[0] > int(self.rows - 1):
                return True
    ...
    def another_brick_in_the_wall(self, brick):
        ...
        if not self.brick_wall[brick.grid_pos[0]]:
            self.brick_wall[brick.grid_pos[0]] = [None] * int(self.columns)
